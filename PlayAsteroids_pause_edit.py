          ...
          # Needs to change GAME_OVER = True and say so
              ...
              for t in targets:
                  if ( (t is not self) and t.is_hit_by(self) ):
                      ...
                      # should report death and lives attribute
                      break
...
class PlayAsteroids(Game):
    ...
    def __init__(self):
        Game.__init__(self,"ASTEROIDS!!!",60.0,45.0,800,580,topology='wrapped',console_lines=6)
    ...
    def handle_keypress(self,event):
        ...
        # later on we may switch the cause of this event to an asteroid impact
            self.ship.explode()
            self.report("EXPLOSIONS!")
        elif event.char == ' ':
            self.ship.shoot()
        elif event.char == 'p':
          if self.PAUSE_GAME == True:
            self.report("Game is paused.  Press 'p' to unpause the game")
          else:
            self.report()
            self.report()
            self.report()
    ...
    def update(self):
        ...
        """if LIVES_COUNT == 3:
          ...
          self.report()
          self.report()
          self.report("GAME OVER.  Press 'q' to quit, ' ' to play again")"""
